...
/**
 * Script sederhana untuk test login dengan PIN 666666 (tanpa database)
 * Jalankan dengan: php test_login_simple.php
 * Pastikan web server sudah berjalan di http://localhost:8000
 */

echo "=== TEST LOGIN SEDERHANA (TANPA DATABASE) ===\n\n";
...
// Test logout
$ch = curl_init($baseUrl . '/panel/logout');
curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
curl_setopt($ch, CURLOPT_POST, true);
curl_setopt($ch, CURLOPT_COOKIEJAR, 'login_cookies.txt');
curl_setopt($ch, CURLOPT_COOKIEFILE, 'login_cookies.txt');
...
$logoutFinalUrl = curl_getinfo($ch, CURLINFO_EFFECTIVE_URL);
curl_close($ch);

if ($logoutHttpCode === 200 || $logoutHttpCode === 302) {
    if (strpos($logoutFinalUrl, '/panel/login') !== false) {
        echo "   - ✅ Logout berhasil! Redirect ke login page\n";
    } else {
        echo "   - ⚠️ Logout response OK tapi redirect tidak sesuai\n";
    }
} else {
    echo "   - ❌ Logout gagal (HTTP {$logoutHttpCode})\n";
}
...
if (strpos($finalUrl, '/panel/shortlinks') !== false && $dashboardHttpCode === 200) {
    ...
    echo "✅ Login dengan PIN 666666 (hardcoded, tidak pakai database)\n";
    echo "✅ Session management yang robust\n";
    echo "✅ Error handling yang baik\n";
    echo "✅ Logout berfungsi\n";
    echo "✅ Flash messages untuk feedback\n\n";
    ...
    echo "5. Gunakan tombol logout untuk keluar\n";
} else {
